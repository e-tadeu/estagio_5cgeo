...
class Projeto8Solucao(QgsProcessingAlgorithm):
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        for lyr in vias.getFeatures():
            ...
            #ge = lyr.geometry()
            if length < alcance_max: 
                smallLines.append(lyr)
                #feicao = QgsFeature()
                #feicao.setGeometry(ge)
                #pointsflag.dataProvider().addFeatures([feicao])
                #pointsflag.updateExtents()
        #QgsProject.instance().addMapLayer(pointsflag)

        #poflag = QgsVectorLayer(f"Point?crs={vias.crs().authid()}",
        #                             "pontos de referencia",
        #                             "memory"
        #                             )
        #poflag.dataProvider().addAttributes([QgsField("id", QVariant.Int)])
        vias.startEditing()
        for linesmall in smallLines:
            ...
            for line in vias.getFeatures(bbox):
                ...
                if point_reference == False: 
                    ...
                    #feicao = QgsFeature()
                    #feicao.setGeometry(pointflag)
                    #poflag.dataProvider().addFeatures([feicao])
                    #poflag.updateExtents()
        
                    #feedback.pushInfo(f'\nA pointflag {pointflag} é do tipo {type(pointflag)}.')
                    ...
                    if geomline2.intersects(pointflag): continue
                        ...
                        #feedback.pushInfo(f'\nA lista {vertices} possui pontos do tipo {type(vertices)}.')
